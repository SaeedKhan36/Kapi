...
/** GitHub rejects a pull request whose body exceeds ~65k characters. */
const PR_BODY_FILE_LIMIT = 40;

function prBody(runId: string, outcome: WorkerOutcome): string {
  const files = outcome.filesChanged;
  const shown = files.slice(0, PR_BODY_FILE_LIMIT);
  ...
  return [
    ...
    ...shown.map((f) => `- \`${f.path}\` (${f.action})`),
    rest > 0 ? `- …and ${rest} more` : "",
    "",
    outcome.incomplete ? "> This branch was cut off before the agent finished. Review carefully.\n" : "",

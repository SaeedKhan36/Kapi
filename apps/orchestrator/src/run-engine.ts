...
import {
  cloneRepo, createSandboxProvider, integrationBranch, isEmptyRepo, seedEmptyRepo,
  type ProviderName, type SandboxProvider,
} from "@kapi/sandbox";
...
export class RunEngine {
  ...
  async #openPullRequests(
    ...
    const pushed = [...outcomes.entries()].filter(([, o]) => o.pushed && o.commits.length > 0);
    if (pushed.length === 0) return;

    const urls: string[] = [];
    for (const [taskId, outcome] of pushed) {
      ...
      try {
        const pr = await openPullRequest(token, ref, {
          ...
          body: [
            ...
            "",
            "### Files changed",
            ...outcome.filesChanged.map((f) => `- \`${f.path}\` (${f.action})`),
            "",
            `Generated by kapi run \`${runId}\`. Review before merging.`,
          ].join("\n"),
        });
        urls.push(pr.html_url);
        await this.store.setTaskStatus(runId, taskId, "review", { });
        this.#emit({ kind: "task", runId, taskId, status: "review", detail: `PR #${pr.number}` });
      } catch (err) {
        const detail = err instanceof Error ? err.message : String(err);
        this.#emit({ kind: "task", runId, taskId, status: "review", detail: `PR failed: ${detail}` });
      }
    }

    if (urls.length > 0) {
      await this.store.updateRun(runId, { prUrl: urls[0] });
      await this.store.saveArtifact(runId, "pull-requests", urls);
      this.#emit({ kind: "status", runId, status: "pr-opened", detail: urls.join(" ") });
    }
  }

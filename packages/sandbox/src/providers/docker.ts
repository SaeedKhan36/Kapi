...
export class DockerProvider implements SandboxProvider {
  ...
  async create(spec: SandboxSpec): Promise<Sandbox> {
    ...
    try {
      ...
      throw new SandboxError(`docker run failed: ${String(cause)}`, this.name, cause);
    }
  }
  ...
  async writeFile(id: string, path: string, content: string) {
    ...
    if (res.exitCode !== 0) {
      throw new SandboxError(`writeFile failed: ${res.stderr}`, this.name);
    }
  }
  ...
  async readFile(id: string, path: string) {
    ...
    if (res.exitCode !== 0) throw new SandboxError(`readFile failed: ${res.stderr}`, this.name);
    return res.stdout;
  }
